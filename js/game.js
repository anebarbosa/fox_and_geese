(function (document) {
  var gansoSelecionado = null,
    ...
    salvarDados = function () {
      localStorage.setItem('vitorias_gansos', informacoes.vitorias.gansos.quantidade);
      localStorage.setItem('vitorias_raposa', informacoes.vitorias.raposa.quantidade);
    },
    carregarDados = function () {
      var vitoriasGansos = localStorage.getItem('vitorias_gansos');
      var vitoriasRaposa = localStorage.getItem('vitorias_raposa');

      if (vitoriasGansos !== null) {
        informacoes.vitorias.gansos.quantidade = parseInt(vitoriasGansos, 10);
        document.getElementById('total_ganso').innerText = informacoes.vitorias.gansos.quantidade;
      }
      if (vitoriasRaposa !== null) {
        informacoes.vitorias.raposa.quantidade = parseInt(vitoriasRaposa, 10);
        document.getElementById('total_raposa').innerText = informacoes.vitorias.raposa.quantidade;
      }
    },

    // Atualiza informações da tela sobre as vitorias
    informacoes = {
      turno: {
        atualizar: function () {
      ...
      gansos: {
        ...
        matar: function () {
          ...
          verificaGansos(); // Verifica o estado dos gansos após matar um
        }
      },
      vitorias: {
        gansos: {
          ...
          adicionar: function () {
            ...
            salvarDados(); // Salva os dados após adicionar uma vitória
          }
        },
        raposa: {
          quantidade: 0,
  ...
  // Carrega os dados quando o script é iniciado
  carregarDados();
  ...
  var exibirPecasJogaveis = function (posicaoPartida) {
    ...
    if (posicaoPartida.x > -1 && posicaoPartida.y > -1) {
      // Obtém todos os espaços vazios próximos
      var i = 0,
        qtd = passos.length;
      for (; i < qtd; i++) {
        if (passos[i] !== null) {
    ...
    if (!possuiMovimentos && turno === 1) {
      // Se não há movimentos e é a vez da Raposa, verifica se a vitória é dos Gansos
      if (!verificaGansos()) {
        return;
      }
      informacoes.vitorias.gansos.adicionar();
  ...
  var verificaGansos = function () {
    ...
    if (gansos.length <= 3) {
      informacoes.vitorias.raposa.adicionar();
  ...
  var mostrarModal = function (titulo, mensagem, callback) {
    ...
    modal.querySelector('.titulo h1').innerText = titulo;
    modal.querySelector('#mensagem').innerText = mensagem;
    modal.classList.add('aberta');
    overlay.classList.add('aberta');
    modal.querySelector('a.fechar').addEventListener('click', function () {
